...
wss.on("connection", (ws: WebSocket, req: any) => {
  ...
  ws.on("message", async (raw) => {
    ...
    // que procurar; `local` e `mcp` já abrem listando, e uma fonte de provedor abria só com a caixa de
    // busca vazia. Quem tinha a conta certa, verificada e vinculada não via tarefa nenhuma e concluía
    // que a integração estava quebrada.
    if (msg.t === "task_provider_list" && typeof msg.sessionId === "string") {
      if (!requireOwner(ws)) return;
      const taskRunnerId = activeRunner(ws);
      // Mesmo portão da busca (fatia D): projeto que declara pasta local não lista de provedor.
      const { source } = sessionTaskSource(taskRunnerId, msg.sessionId);
      if (source.kind !== "provider" || !source.ready) {
        send(ws, { t: "task_provider_results", sessionId: msg.sessionId, results: [], error: source.reason || "este projeto não usa provedor como fonte de tarefas", code: source.code });
        return;
      }
      const { resolved } = resolveSessionTaskConnection(msg.sessionId, false, taskRunnerId);
      if ("refusal" in resolved) { send(ws, { t: "task_provider_results", sessionId: msg.sessionId, results: [], error: resolved.refusal.message, code: resolved.refusal.code }); return; }
      const t = providerSignal();
      try {
        const results = await listProviderTasks(resolved.connection.provider, { config: resolved.connection.config, secret: resolved.secret, secret2: resolved.secret2, signal: t.signal });
        send(ws, { t: "task_provider_results", sessionId: msg.sessionId, results, connection: { id: resolved.connection.id, label: resolved.connection.label, identity: resolved.connection.identity || null } });
      } catch (e: any) { send(ws, { t: "task_provider_results", sessionId: msg.sessionId, results: [], error: String(e?.message ?? e) }); }
      finally { t.finish(); }
      return;
    }
