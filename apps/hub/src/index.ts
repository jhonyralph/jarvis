...
interface PendingRunnerUpdate { requestId: string; targetCommit: string; requestedAt: number; state: "queued" | "sent" | "awaiting_restart" | "blocked"; force?: boolean; fromCommit?: string; lastAttemptAt?: number; lastError?: string; awaitingSince?: number; stalled?: boolean; failures?: number; lastNote?: string; deliveries?: number; scriptSha256?: string; scriptRanSha256?: string; scriptFromHub?: boolean; stalledNotifiedAt?: number; /** Last out-of-band phase the runner's detached updater phoned home (Fase 2 UPD-01): even when the runner process dies mid-update and never sends update_done, this records WHERE it died. */ lastPhase?: string; lastReportAt?: number; /** Últimas linhas do log do updater na falha — o rastro que diz QUAL comando quebrou, não só que algo quebrou. */ lastLogTail?: string; }
...
function queueRunnerUpdate(runnerId: string, targetCommit: string, opts?: { force?: boolean }): PendingRunnerUpdate {
  ...
  if (existing && commitMatches(existing.targetCommit, targetCommit) && existing.state !== "blocked") {
    ...
    // Forçar é decisão NOVA do dono: reabre o disjuntor de entrega. Sem isto, o caminho em massa
    // (que força aqui e entrega sem `force`) deixaria uma máquina no teto presa para sempre.
    if (opts?.force && (existing.stalled || existing.deliveries)) { existing.deliveries = 0; existing.stalled = false; existing.stalledNotifiedAt = undefined; savePendingRunnerUpdates(); }
    return existing;
  }
...
function normalizePendingRunnerUpdate(rc: RunnerConn): PendingRunnerUpdate | null {
  ...
  pending.deliveries = 0; pending.stalled = false; pending.stalledNotifiedAt = undefined;
...
function deliverPendingRunnerUpdate(rc: RunnerConn, opts?: { force?: boolean; allowBlocked?: boolean; retryNow?: boolean }): boolean {
  ...
  // Disjuntor da ENTREGA: cada reenvio derruba a máquina (o updater mata o runner para mexer no
  // checkout). Um pedido que não avança de estado vira um círculo que se alimenta sozinho — e o
  // watchdog existente não pega, porque ele vigia `awaiting_restart` offline e este caso fica em
  // `sent`. Quem reabre é o dono, enfileirando forçado (ver queueRunnerUpdate) — NÃO o `force` deste
  // opts, que também carrega o reparo automático de mesmo-SHA da quarentena de protocolo.
  const entrega = runnerUpdateDeliveryDecision({ deliveries: pending.deliveries });
  if (!entrega.deliver) {
    if (!pending.stalled) {
      pending.stalled = true; pending.stalledNotifiedAt = Date.now(); pending.lastError = entrega.reason;
      savePendingRunnerUpdates();
      const label = runnerLabels[rc.id] || rc.info.host || rc.id;
      log.warn("update_delivery_breaker", { runnerId: rc.id, label, deliveries: pending.deliveries, targetCommit: pending.targetCommit });
      notifyEvent("machine", `${label}: atualização em círculo`, `${entrega.reason} Use "forçar" no painel depois de resolver a máquina.`);
      updateMachineNotice(rc.id, { ok: false, stalled: true, state: "stalled", log: entrega.reason });
    }
    return false;
  }
  ...
  if (sent) { if (!pending.fromCommit && rc.info.commit) pending.fromCommit = rc.info.commit; pending.state = "sent"; pending.deliveries = (pending.deliveries || 0) + 1; pending.lastAttemptAt = Date.now(); pending.lastError = undefined; pending.stalled = false; pending.stalledNotifiedAt = undefined; pending.awaitingSince = undefined; savePendingRunnerUpdates(); updateMachineNotice(rc.id, { state: "sent", queued: true, ok: false, log: "solicitação entregue; máquina drenando" }); }
  return sent;
}
...
function completePendingRunnerUpdate(rc: RunnerConn, m: any): void {
  ...
  if (pending && (!m.requestId || m.requestId === pending.requestId)) {
    ...
    if (m.ok) { pending.state = "awaiting_restart"; pending.deliveries = 0; pending.lastError = undefined; pending.awaitingSince = Date.now(); pending.stalled = false; pending.stalledNotifiedAt = undefined; }
  ...
  if (m.t === "history") {
    ...
    if (Array.isArray(m.liveActivity) && m.liveActivity.length) activityBuf.set(liveKey, m.liveActivity.slice(0, 1_200));
      ...
      send(request.socket, {
        ...
        total: m.total,
        messages: messages.map((x: any) => ({ sessionId: m.sessionId, role: x.role, text: x.text, ts: x.ts, agent: x.agent || m.agent, speaker: x.speaker, images: x.images, files: x.files, usage: x.usage, name: x.name, detail: x.detail, path: x.path, adds: x.adds, dels: x.dels, rows: x.rows, activity: x.activity, contextManifest: x.contextManifest, interrupted: x.interrupted, restored: x.restored })),
        files: m.files,
      });
...
/** Reconstroi no store os turnos que existem no transcript nativo e sumiram daqui — ver
 *  `history-gaps.ts` para o porque e para o filtro de falso positivo. Diferente de
 *  `reconcileFromNative`, que so completa a CAUDA, este preenche buraco no meio: e o formato do
 *  incidente em que um cancelamento apagou pergunta e resposta de um turno inteiro.
 *
 *  Silencioso por escolha: nao levanta erro nem avisa quando nao ha nada a fazer (o caso comum).
 *  O que restaura fica marcado com `restored`, entao a origem nunca vira adivinhacao. */
function backfillHistoryGaps(s: ReturnType<typeof store.ensure>, nh: { messages: Array<{ role: string; text: string; ts: number; activity?: unknown[] }> } | null): number {
  if (!nh || activeRuns.has(s.id) || isNativeId(s.id)) return 0;
  try {
    const gaps = findHistoryGaps(store.history(s.id), nh.messages);
    if (!gaps.length) return 0;
    const restored = gaps.flatMap((g) => [
      { role: "user" as const, text: g.user.text, ts: g.user.ts, agent: s.agent },
      { role: "assistant" as const, text: g.assistant.text, ts: g.assistant.ts, agent: s.agent, activity: g.assistant.activity as unknown[] | undefined },
    ]);
    const n = store.insertRestored(s.id, restored);
    if (n) { log.info("history_backfill", { sessionId: s.id, agent: s.agent, turns: gaps.length, messages: n }); pushSessions(); }
    return n;
  } catch (e) {
    // Recuperacao e um bonus: ela nunca pode impedir a conversa de abrir.
    log.warn("history_backfill_failed", { sessionId: s.id, error: String((e as any)?.message ?? e) });
    return 0;
  }
}
...
wss.on("connection", (ws: WebSocket, req: any) => {
  ...
  ws.on("message", async (raw) => {
    ...
    if (msg.t === "open" && typeof msg.sessionId === "string") {
      ...
      // Turno que sumiu do MEIO do historico volta aqui (reconcileFromNative so cobre a cauda). Roda
      // no OPEN, nao no boot: o transcript ja foi lido logo acima para esta sessao, entao o custo e
      // zero — varrer todas as sessoes no boot e exatamente o que ja travou o event loop deste Hub.
      backfillHistoryGaps(s, nh);
      const all = store.history(s.id);
      const su = sessionUsage(s.id), lastUsage = [...all].reverse().find((m: any) => m.usage)?.usage;
    ...
    // "As MINHAS tarefas abertas" no provedor do projeto — a porta que faltava. Buscar exige saber o
    // que procurar; `local` e `mcp` ja abrem listando, e uma fonte de provedor abria so com a caixa de
    // busca vazia. Quem tinha a conta certa, verificada e vinculada nao via tarefa nenhuma e concluia
    // que a integracao estava quebrada.
    if (msg.t === "task_provider_list" && typeof msg.sessionId === "string") {
      if (!requireOwner(ws)) return;
      const taskRunnerId = activeRunner(ws);
      const cursor = typeof msg.cursor === "string" ? msg.cursor : "";
      const estado = typeof msg.state === "string" ? msg.state : "";
      const responder = (extra: Record<string, unknown>): void => { send(ws, { t: "task_provider_results", sessionId: msg.sessionId, state: estado, ...extra }); };
      // Mesmo portao da busca (fatia D): projeto que declara pasta local nao lista de provedor.
      const { source } = sessionTaskSource(taskRunnerId, msg.sessionId);
      if (source.kind !== "provider" || !source.ready) {
        responder({ results: [], error: source.reason || "este projeto não usa provedor como fonte de tarefas", code: source.code });
        return;
      }
      const { resolved } = resolveSessionTaskConnection(msg.sessionId, false, taskRunnerId);
      if ("refusal" in resolved) { responder({ results: [], error: resolved.refusal.message, code: resolved.refusal.code }); return; }
      const t = providerSignal();
      const chamada = { config: resolved.connection.config, secret: resolved.secret, secret2: resolved.secret2, signal: t.signal };
      try {
        // Os estados so na PRIMEIRA pagina: eles mudam raramente e o filtro ja esta desenhado quando
        // alguem pede "carregar mais" — repetir a consulta por pagina seria uma ida de rede por rolagem.
        const [pagina, estados] = await Promise.all([
          listProviderTasks(resolved.connection.provider, { ...chamada, state: estado, cursor }),
          cursor ? Promise.resolve(null) : listProviderStates(resolved.connection.provider, chamada).catch(() => null),
        ]);
        responder({ results: pagina.tasks, cursor: pagina.cursor, ...(estados ? { states: estados } : {}),
          connection: { id: resolved.connection.id, label: resolved.connection.label, identity: resolved.connection.identity || null } });
      } catch (e: any) { responder({ results: [], error: String(e?.message ?? e) }); }
      finally { t.finish(); }
      return;
    }
    ...
    if (msg.t === "update_apply") {
      ...
      if (typeof msg.runnerId === "string" && msg.runnerId) {
        ...
        else { const p = pendingRunnerUpdates[rc.id], online = !!rc.ws && rc.ws.readyState === WebSocket.OPEN, state = p?.stalled ? "stalled" : (online ? (p?.state || "sent") : "queued"); send(ws, { t: "update_machine", runnerId: rc.id, label, ok: false, stalled: !!p?.stalled, state, queued: !p?.stalled, log: p?.stalled ? (p.lastError || "reenvio pausado") : (online ? "atualização já está em andamento" : "máquina offline — atualização guardada para a próxima conexão") }); }
        return;
      }
