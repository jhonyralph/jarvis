    ...
    function openFile(path,action){ if(E.workPanel&&!E.workPanel.classList.contains('hidden'))closeWorkPanel(); E.filePanel.classList.remove('hidden'); E.fileName.textContent=path.split(/[\\/]/).pop()||path; E.fileName.title=path;
    ...
    function renderWorkTree(preserveFocus){ const active=document.activeElement&&document.activeElement.closest&&document.activeElement.closest('.worknode'), focusId=preserveFocus&&active&&active.dataset.id; E.workTree.innerHTML=''; const rows=workTreeRows();
      ...
      rows.forEach(({n,level})=>{ const kids=workChildren(n.executionId).length, b=document.createElement('button'); b.type='button'; b.className='worknode'; b.dataset.id=n.executionId; b.dataset.state=n.state||'unknown'; b.setAttribute('role','treeitem'); b.setAttribute('aria-level',String(level)); b.setAttribute('aria-selected',String(n.executionId===workSelected)); if(kids)b.setAttribute('aria-expanded',String(!workCollapsed.has(n.executionId))); b.style.paddingLeft=(8+(level-1)*15)+'px';
        const meta=[n.agent,n.model,n.effort].filter(Boolean).join(' · ')||n.kind||'trabalho', state=(n.archivedAt?'Arquivado · ':'')+workStateLabel(n.state); b.innerHTML=`<span class="wbranch">${kids?(workCollapsed.has(n.executionId)?'▸':'▾'):''}</span><span class="wnmain"><span class="wntitle">${esc(String(n.title||n.summary||n.executionId))}</span><span class="wnmeta">${esc(meta)} · ${workDuration(n)}</span></span><span class="wnstate">${state}</span>`;
    ...
    function renderWorkDetail(){ const n=workNodes.get(workSelected); if(!n){ E.workCrumb.textContent='';E.workNodeTitle.textContent='Selecione um trabalho';E.workNodeState.textContent='';E.workDetailBody.innerHTML='<div class="workempty"><span class="weicon">🧩</span><span>Selecione um trabalho para acompanhar.</span></div>';return; }
      ...
      let h='', conn=workConnections.get(n.runnerId); if(conn&&conn!=='online')h+=`<div class="worknotice"><b>Máquina ${esc(conn)}.</b> Esta é a última visão persistida; os controles podem ficar indisponíveis até a reconciliação.</div>`; if(n.state==='orphaned')h+='<div class="worknotice err"><b>Conexão perdida.</b> O estado final ainda não foi observado; o Jarvis tentará reconciliar sem marcar cancelamento por inferência.</div>'; if(n.state==='unknown')h+='<div class="worknotice">Estado parcial: o provider ainda não publicou lifecycle suficiente.</div>'; if(n.truncated)h+='<div class="worknotice">Transcript truncado. O que aparece abaixo é apenas a parte preservada.</div>'; if(n.summary)h+=`<div class="worknotice" style="border-color:var(--line);background:#ffffff07;color:var(--text)">${esc(String(n.summary))}</div>`;
      ...
      else if(workTab==='transcript'){ const level=cap.transcript||'none'; h+=`<div class="worknotice">Nível publicado pelo adapter: <b>${esc(level)}</b>. O painel nunca apresenta raciocínio privado como transcript.</div>`; if(n.prompt)h+=`<div class="worksection">Instrução delegada</div><div class="workevent"><div class="wetext">${esc(String(n.prompt))}</div></div>`; const transcript=events.filter(e=>e.kind==='message'||e.kind==='summary'||(e.kind==='thinking'&&e.published)); h+=transcript.length?transcript.map(workEventHtml).join(''):`<div class="workempty"><span class="weicon">💬</span><span>${level==='none'?'Este adapter não fornece transcript.':'Nenhuma mensagem publicada ainda.'}</span></div>`; if(workTranscriptCursor.get(n.executionId))h+='<button type="button" class="ghost" data-transcript-more style="width:100%;margin-top:8px">Carregar mais do transcript</button>'; }
