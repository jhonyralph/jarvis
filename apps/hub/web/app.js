    ...
    function openSettingsHelp(name){ const info=SETTINGS_PANEL_HELP[name||settingsCurrentPanel()]||SETTINGS_PANEL_HELP.geral; helpDialog(info.title,info.body,info.items); }
    ...
    function renderPersonalSuggestions(rows,errors=[],purpose,diagnostics=[]){personalSuggestions=rows||[];E.personalResults.innerHTML='';if(purpose&&E.personalPurpose){E.personalPurpose.value=purpose;renderPersonalReferenceOptions();}if(!personalSuggestions.length)E.personalResults.textContent=errors.length?errors.map(e=>e.error).join(' · '):personalText(['Nenhum resultado encontrado nas fontes autorizadas.','No results from authorized sources.','No hay resultados en las fuentes autorizadas.']);personalSuggestions.forEach(s=>E.personalResults.appendChild(createPersonalSuggestionElement(s,purpose||E.personalPurpose.value)));renderPersonalDiagnostics(diagnostics);if(!E.personalMap.classList.contains('hidden'))renderPersonalMap();}
    ...
    function settingsLoadPanel(name){
      ...
      else if(['assistente','fontes','dados'].includes(name)){ requestPersonalState(); }
    }
    ...
    if(E.setSection)E.setSection.onchange=()=>{if(E.setSearch)E.setSearch.value='';settingsSearch('');settingsGoto(E.setSection.value);};
    ...
    if(E.fwSeed) E.fwSeed.onclick=()=>tx({t:'framework_seed'});
    ...
    function voiceMatchesLang(v){
      ...
      if(typeof v==='string'){ const m=/^([a-z]{2})_/i.exec(v||''); return m ? m[1].toLowerCase()===cur : false; }
    ...
    function renderWorkTree(preserveFocus){ workDefaultCollapse(); const active=document.activeElement&&document.activeElement.closest&&document.activeElement.closest('.worknode'), focusId=preserveFocus&&active&&active.dataset.id; E.workTree.innerHTML=''; const rows=workTreeRows();
      ...
      rows.forEach(({n,level})=>{ const kids=workChildren(n.executionId).length, root=!n.parentExecutionId, b=document.createElement('button'); b.type='button'; b.className='worknode'; b.dataset.id=n.executionId; b.dataset.state=n.state||'unknown'; b.setAttribute('role','treeitem'); b.setAttribute('aria-level',String(level)); b.setAttribute('aria-selected',String(n.executionId===workSelected)); if(kids)b.setAttribute('aria-expanded',String(!workCollapsed.has(n.executionId))); b.style.paddingLeft=(8+(level-1)*15)+'px';
        const role=root&&kids?'orquestrador':n.agent, meta=[role,n.model,n.effort].filter(Boolean).join(' · ')||n.kind||'trabalho', state=(n.archivedAt?'Arquivado · ':'')+workStateLabel(n.state); b.innerHTML=`<span class="wbranch" title="${kids?'Expandir/recolher':''}">${kids?(workCollapsed.has(n.executionId)?'▸':'▾'):''}</span><span class="wnmain"><span class="wntitle">${esc(String(n.title||n.summary||n.executionId))}</span><span class="wnmeta">${esc(meta)} · ${workDuration(n)}</span></span><span class="wnstate">${state}</span>`;
    ...
    function connect(){ ws=new WebSocket((location.protocol==='https:'?'wss://':'ws://')+location.host);
      ...
      ws.onmessage=(e)=>{ const m=JSON.parse(e.data);
        ...
        else if(m.t==='framework_seeded'){ if(m.ok){ tx({t:'framework_cfg'}); toast(`Pacote base: ${(m.imported||[]).length} instalado(s), ${(m.skipped||[]).length} preservado(s).`); } else toast('Erro: '+(m.error||'falha')); }
