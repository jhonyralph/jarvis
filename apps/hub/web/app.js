    ...
    function fwDoSave(){ const path=fwEditMode==='new'?((E.fwEditPath&&E.fwEditPath.value||'').trim()):fwEditPath; if(!path){ toast('Informe o caminho (ex.: commands/plan.md)'); return; } fwSavingContent=E.fwEditBody?E.fwEditBody.value:''; if(E.fwEditSave)E.fwEditSave.disabled=true; tx({t:'framework_save',path:path,content:fwSavingContent}); }
    ...
    if(E.fwEditDelete) E.fwEditDelete.onclick=()=>{ if(!fwEditPath){ fwCloseEdit(true); return; } if(!confirm('Excluir '+fwEditPath+'?')) return; tx({t:'framework_delete',path:fwEditPath}); };
    ...
    function connect(){ ws=new WebSocket((location.protocol==='https:'?'wss://':'ws://')+location.host);
      ...
      ws.onmessage=(e)=>{ const m=JSON.parse(e.data);
        ...
        if(m.t==='authinfo'){ showGate(m.claimed); }
        ...
        else if(m.t==='framework_file'){ fwShowFile(m.path||'', m.content||''); }
        else if(m.t==='framework_saved'){ if(m.ok){ tx({t:'framework_cfg'}); tx({t:'framework_inventory'});
          ...
          else { if(E.fwEditSave)E.fwEditSave.disabled=false; toast('Erro: '+(m.error||'falha')); } }
