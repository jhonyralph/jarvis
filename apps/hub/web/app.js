    ...
    let fwSourcesCache=[], fwPreviewToken='', fwPreviewBlocked=false;
    ...
    const fwKindLabel=(k)=>({command:'comando',skill:'skill',instructions:'instruções',other:'outro'})[k]||esc(k||'');
    ...
    function renderFwInventory(inv,scan,validation){
      ...
      E.fwInventory.innerHTML=files.length
        ? '<table style="width:100%;border-collapse:collapse">'+files.map(f=>'<tr><td style="padding:2px 6px 2px 0">'+esc(f.path)+'</td><td class="mut" style="padding:2px 6px">'+fwKindLabel(f.kind)+'</td><td style="padding:2px 6px;text-align:right">'+(f.tokens||0)+' tk</td><td style="padding:2px 0;text-align:right">'+fwStatusBadge(f.status)+'</td></tr>').join('')+'</table>'
        : '<div class="mut">Nenhum arquivo ainda. Use o pacote base, o editor ou importe um pacote.</div>';
    ...
    if(E.fwRefresh) E.fwRefresh.onclick=()=>tx({t:'framework_inventory'});
    if(E.fwZipBtn) E.fwZipBtn.onclick=()=>E.fwZip&&E.fwZip.click();
    if(E.fwZip) E.fwZip.onchange=()=>{ const f=E.fwZip.files&&E.fwZip.files[0]; if(!f) return; if(f.size>18*1024*1024){ toast('Arquivo muito grande'); E.fwZip.value=''; return; } const r=new FileReader(); r.onload=()=>{ const b64=String(r.result||'').split(',').pop()||''; tx({t:'framework_import_zip',name:f.name,dataB64:b64}); toast('Verificando pacote…'); E.fwZip.value=''; }; r.onerror=()=>{ toast('Falha ao ler o arquivo'); E.fwZip.value=''; }; r.readAsDataURL(f); };
    if(E.fwGhBtn) E.fwGhBtn.onclick=()=>{ const v=(E.fwGh.value||'').trim(); if(!v){ toast('Informe owner/repo ou URL do GitHub'); return; } toast('Baixando e verificando…'); tx({t:'framework_import_github',source:v}); };
    ...
    if(E.fwPreviewApply) E.fwPreviewApply.onclick=()=>{ if(!fwPreviewToken) return; tx({t:'framework_import_apply',token:fwPreviewToken,mode:E.fwPreviewMode.value,force:!!(E.fwPreviewForce&&E.fwPreviewForce.checked)}); E.fwPreviewApply.disabled=true; };
    if(E.fwSources) E.fwSources.addEventListener('click',e=>{ const upd=e.target.closest('.fw-src-upd'), del=e.target.closest('.fw-src-del'); if(upd){ toast('Buscando atualização…'); tx({t:'framework_update_check',id:upd.dataset.id}); } else if(del){ tx({t:'framework_source_remove',id:del.dataset.id}); } });
    ...
    function connect(){ ws=new WebSocket((location.protocol==='https:'?'wss://':'ws://')+location.host);
      ...
      ws.onmessage=(e)=>{ const m=JSON.parse(e.data);
        ...
        if(m.t==='authinfo'){ showGate(m.claimed); }
        ...
        else if(m.t==='framework_saved'){ if(m.ok){ if(m.deleted){E.fwPath.value='';E.fwEditor.value='';} tx({t:'framework_cfg'}); tx({t:'framework_inventory'}); toast(m.deleted?'Arquivo excluído':'Arquivo salvo'); } else toast('Erro: '+(m.error||'falha')); }
          ...
          if(Array.isArray(m.results)){ m.results.forEach(r=>{ fwMachineStatus[r.runnerId]={label:r.label||r.runnerId,state:r.state}; }); if(typeof m.version==='number')E.fwVersion.textContent='Versão atual: '+m.version; renderFwStatus(); tx({t:'framework_inventory'}); }
