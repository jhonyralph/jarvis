    ...
    function connect(){ ws=new WebSocket((location.protocol==='https:'?'wss://':'ws://')+location.host);
      ...
      ws.onmessage=(e)=>{ const m=JSON.parse(e.data);
        ...
        else if(m.t==='workflow_list'){ fwArrived(); if(m.ok){ fwWfCache=m; renderFwWorkflows(m); fwLog('fluxos: '+((m.workflows||[]).length)+' salvo(s), '+((m.candidates||[]).length)+' skill(s) detectável(is)'); } else { fwLog('✖ fluxos: '+esc(m.error||'falha'),'#f87171'); toast('Fluxos: '+(m.error||'falha')); } }
