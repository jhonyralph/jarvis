 ...
 * mais antigo (que vinha do INÍCIO) — uma sessão retomada em `projeto-alfa` aparecia como sendo de
 * `ProjetoBeta`. E o mesmo `cwd` é o que retoma a sessão e o que resolve a fonte de tarefas,
...
mkdirSync(join(CLAUDE, "C--Projetos-ProjetoBeta"), { recursive: true });
mkdirSync(join(CLAUDE, "C--Projetos-ia-framework"), { recursive: true });
...
writeFileSync(join(CLAUDE, "C--Projetos-ProjetoBeta", `${MUDOU}.jsonl`), jsonl([
  ...turn("C:\\Projetos\\ProjetoBeta", "começo aqui", 1),
  ...turn("C:\\Projetos\\projeto-alfa", "agora estou no projeto-alfa", 2),
  ...turn("C:\\Projetos\\projeto-alfa", "seguimos no projeto-alfa", 3),
  { type: "ai-title", aiTitle: "Ajustes no fluxo de assinatura do projeto-alfa" },
]));
...
writeFileSync(join(CLAUDE, "C--Projetos-ia-framework", `${SUBPASTA}.jsonl`), jsonl([
  ...turn("C:\\Projetos\\ia-framework", "vamos começar", 1),
  ...turn("C:\\Projetos\\ia-framework\\cli", "entrei na cli", 2),
...
writeFileSync(join(CLAUDE, "C--Projetos-ia-framework", `${PARADA}.jsonl`), jsonl([
  ...turn("C:\\Projetos\\ia-framework", "tudo aqui mesmo", 1),
...
writeFileSync(join(CODEX, "2026", "08", "20", `rollout-2026-08-20T12-00-00-${CODEX_ID}.jsonl`), jsonl([
  { type: "session_meta", payload: { session_id: CODEX_ID, cwd: "C:\\Projetos\\ProjetoBeta", title: "sessão do codex" } },
  { type: "response_item", timestamp: "2026-08-20T12:00:00Z", payload: { type: "message", role: "user", content: "oi" } },
  { type: "turn_context", payload: { cwd: "C:\\Projetos\\projeto-alfa" } },
]));
...
test("resolveNativeCwd: retomada em outro projeto muda o projeto; subpasta não", () => {
  assert.equal(resolveNativeCwd("C:\\Projetos\\ProjetoBeta", "C:\\Projetos\\projeto-alfa"), "C:\\Projetos\\projeto-alfa");
  assert.equal(resolveNativeCwd("C:\\Projetos\\ia-framework", "C:\\Projetos\\ia-framework\\cli"), "C:\\Projetos\\ia-framework",
...
test("resolveNativeCwd: barra e caixa não inventam projeto diferente", () => {
  assert.equal(resolveNativeCwd("C:/Projetos/app", "C:\\Projetos\\app\\src"), "C:/Projetos/app", "separador diferente é o mesmo caminho");
  assert.equal(resolveNativeCwd("C:\\Projetos\\app\\", "C:\\Projetos\\app"), "C:\\Projetos\\app\\", "barra no fim não é outro projeto");
  // Prefixo de NOME não é prefixo de CAMINHO: "app-v2" não está dentro de "app".
  assert.equal(resolveNativeCwd("C:\\Projetos\\app", "C:\\Projetos\\app-v2"), "C:\\Projetos\\app-v2");
});

test("resolveNativeCwd: quando só um lado existe, ele responde", () => {
  assert.equal(resolveNativeCwd("", "C:\\Projetos\\projeto-alfa"), "C:\\Projetos\\projeto-alfa");
  assert.equal(resolveNativeCwd("C:\\Projetos\\app", ""), "C:\\Projetos\\app");
...
test("navegar para subpasta NÃO cria um segundo projeto na lista", () => {
  assert.equal(porId("claude:" + SUBPASTA)!.cwd, "C:\\Projetos\\ia-framework");
});

test("sessão que nunca mudou de pasta continua exatamente como era", () => {
  assert.equal(porId("claude:" + PARADA)!.cwd, "C:\\Projetos\\ia-framework");
});
