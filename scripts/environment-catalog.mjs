...
const CATALOG = [
  ...
  ...entries("Agentes, busca e execucoes", {
    ...
    ["JARVIS_CODEX_PRICE_IN", { defaultValue: "none; the per-model table in codex-pricing.ts is used instead", format: "non-negative USD per 1M non-cached input tokens", provider: "Jarvis Codex estimator", description: "Operator override for the local estimate. Setting any JARVIS_CODEX_PRICE_* value replaces the per-model table with flat rates for EVERY model (no long-context or fast-tier multipliers). It does not alter OpenAI billing.", example: "1.25", cost: "display estimate only; source labels it ballpark, not a quoted price" }],
    ["JARVIS_CODEX_PRICE_CACHED", { defaultValue: "JARVIS_CODEX_PRICE_IN / 10 when the override is active", format: "non-negative USD per 1M cached input tokens", provider: "Jarvis Codex estimator", description: "Cached-input coefficient of the flat override.", example: "0.125", cost: "display estimate only; not a quoted price" }],
    ["JARVIS_CODEX_PRICE_OUT", { defaultValue: "10 when the override is active", format: "non-negative USD per 1M output tokens", provider: "Jarvis Codex estimator", description: "Output coefficient of the flat override.", example: "10", cost: "display estimate only; not a quoted price" }],
    ["JARVIS_CODEX_PRICING_VERSION", { defaultValue: "jarvis-codex-prices-2026-09", format: "free-form version label", provider: "Jarvis Codex estimator", description: "Labels the estimate table shown in usage provenance.", example: "jarvis-codex-prices-2026-09", cost: "display metadata only" }],
    ["JARVIS_CODEX_LONG_CONTEXT_MODE", { defaultValue: "whole", format: "whole | excess", provider: "Jarvis Codex estimator", description: "How the long-context surcharge is estimated once a turn crosses the model's input threshold: 'whole' reprices the entire prompt (literal reading of the published wording; never under-reports), 'excess' surcharges only the tokens above the threshold. Switch to 'excess' only after measuring against a real invoice.", example: "whole", cost: "display estimate only; changes the estimate by ~50% on surcharged turns" }],
