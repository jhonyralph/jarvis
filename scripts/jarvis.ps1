...
function Show-Invite($r) {
  Write-Host ''
  Write-Host '  Codigo do convite:' -ForegroundColor Cyan
  ...
  Write-Host ''
  Write-Host '  No app: recarregue e cole o codigo na tela de pareamento.' -ForegroundColor DarkGray
}
...
try {
  switch ($cmd) {
    ...
    'machine'    {
      ...
      Write-Host ''
      Write-Host '  Token da maquina:' -ForegroundColor Cyan
      Write-Host "    $($r.token)" -ForegroundColor White
      Write-Host '  Hub:' -ForegroundColor Cyan
      Write-Host "    $($r.hub)" -ForegroundColor White
      Write-Host ''
      Write-Host '  Na maquina nova (com o repo jarvis clonado), instale como servico:' -ForegroundColor DarkGray
      ...
      Write-Host '  (autostart via Task Scheduler / launchd / systemd; a maquina aparece no seletor do Hub)' -ForegroundColor DarkGray
    }
    ...
    'update-rollback'  { Write-Host 'Revertendo para a versao anterior (vai reiniciar)...' -ForegroundColor Yellow; Invoke-RestMethod -Method Post "$base/admin/update/rollback" | ConvertTo-Json -Depth 5 }
    ...
    'claimcode'  { $r = Invoke-RestMethod "$base/admin/claimcode"; if ($r.claimed) { Write-Host 'Ja reivindicado. Use "owner" para gerar um convite de dono.' } else { Write-Host "Claim code: $($r.code)" -ForegroundColor Cyan } }
  ...
  Write-Host "Falha ao falar com o Hub em $base - o Jarvis esta rodando?" -ForegroundColor Red
